...
class SphereEntropyModel(nn.Module):
    ...
    def dequantize(inputs, means=None):
        outputs = inputs.type_as(means) + means if means is not None else inputs.float()
        return outputs
...
class SphereEntropyBottleneck(SphereEntropyModel):
    r"""Entropy bottleneck layer for spherical data
    """
    def __init__(self,
                 channels,
                 *args,
...
class SphereGaussianConditional(SphereEntropyModel):
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def forward(self, inputs, scales, means=None, training=None, skip_quant:bool=False):
        "type: (Tensor, Tensor, Optional[Tensor]) -> Tuple[Tensor, Tensor]"
        if training is None:
            training = self.training
        outputs = self.quantize(inputs, "noise" if training else "dequantize", means) if not skip_quant else inputs
