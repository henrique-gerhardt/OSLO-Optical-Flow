"""Vendored verbatim from longliangLiu/PriOr-Flow (ICCV 2025), file ``core/utils/projection_prim_ortho.py``.
...
without checking with the authors. sha256 of the original: 9d9bb8e8e94444ef
"""
import numpy as np
import torch
...
from .my_cycle_sample import cycle_grid_sample
...
def generate_plane_grid(tensor_size):
    ...
    plane_grid = plane_grid.to(_DEVICE)  # B x 2 x H x W
    return plane_grid
        ...
        if axis == 'x':
            R = R @ torch.tensor(
                [[1, 0, 0],
                 ...
                 [0, sin_theta, cos_theta]]).to(_DEVICE)
        elif axis == 'y':
            R = R @ torch.tensor(
                [[cos_theta, 0, sin_theta],
                 [0, 1, 0],
                 [-sin_theta, 0, cos_theta]]).to(_DEVICE)
        elif axis == 'z':
            R = R @ torch.tensor(
...
def Cartesian2Spherical(Cartesian_grid):
    ...
    return Spherical_grid.to(_DEVICE)
...
def bilinear_interpolate(input_tensor, grid):
    ...
    mask = torch.autograd.Variable(torch.ones(input_tensor.size())).to(_DEVICE)
...
def cycle_interpolate(input_tensor, grid):
    ...
    mask = torch.autograd.Variable(torch.ones(input_tensor_padding.size())).to(_DEVICE)
    mask = F.grid_sample(mask, grid, align_corners=True)
