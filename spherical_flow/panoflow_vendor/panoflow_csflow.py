"""Vendored inference-only network from PanoFlow(CSFlow) for the OSLO universality table.

Source: MasterHow/PanoFlow, opticalflow/core/model/external/panoflow_csflow.py
(https://github.com/MasterHow/PanoFlow), MIT License, Copyright (c) 2022 Hao.
Copied verbatim (only this docstring added) so PanoFlow(CSFlow) checkpoints can be
scored through OSLO's spherical geodesic metric. The CFE (Cyclic Flow Estimation)
inference wrapper lives in spherical_flow/panoflow_adapter.py, mirroring the repo's
opticalflow/api/evaluate.py::validate_flow360_cfe. No functional changes.
"""
import torch
